...
class TicketMaker:
  """ Clase para la creacion de Etiquetas 
    ...
    Puede crear paginas completas de etiquetas o una base de datos
    para etiquetas individuales.
  """
  def __init__(self, etiquetas, titulo, ruta, config, posicion):
    self.etiqueta_a_imprimir = [self.separar_lista(elem) for elem in etiquetas]
    self.titulo = titulo
    self.ruta = ruta
    self.config = config
    self.posicion = posicion

  def separar_lista(self, str_dict: dict):
    ...
    clasif = self.separar_clasificacion(str_dict["CLASS"])
  ...
  def separar_clasificacion(self, STR:str):
    ...
    return lista_salida
  def max_len_list(self, lista: list):
    max_len = 0
    for texto in lista:
      max_len = len(texto) if len(texto) > max_len else max_len
    return max_len
  
  def imprimir_etiqueta(self, etiqueta: list, config: dict, ruta: str, num):
    ''' Genera una imagen PNG de la Etiqueta de un Libro
      ...
      RETORNA
      -------
      El path de la imagen generada
    '''
    #* Configuración de la Etiqueta
    IW, IH = config['INDIW'], config['INDIH']
    ...
    max_len = self.max_len_list(etiqueta)
  ...
  def crear_base_datos_etiquetas(self):
    ruta = self.ruta
    titulo = self.titulo
    lista_a_imprimir = self.etiqueta_a_imprimir
...
def image_viewer(lista_de_rutas: list, flag: bool):
  ...
  lista_de_rutas = lista_de_rutas[:5] if not flag else lista_de_rutas
