...
""" 
TODO Ajustar Tamaño de la APP es demasiado grande
TODO Ajustar el encabezado 1 cm de margen
TODO generar un solo pdf con tamaño carta
...
def vetana_modify(STR_clas):
	...
	window = sg.Window("Modificar una Etiqueta", main_layout, element_justification="c")
...
def ventana_config():
	...
	window = sg.Window("Generador de Etiquetas", layout, element_justification="c")
...
def ventana_elemento():
	...
	window = sg.Window(
		"Generador de Etiquetas", layout, element_justification="c"
	)  # Creacion de la ventana
	...
	while True:
		...
		if event == "Exportar":
			if ruta_folder != "":  # Revisamos que exista una ruta de folder
				...
				if len(selected) != 0:
					crear_reporte(
						tabla_modify, ruta_folder, today_date
					)  # Con la tabla de modificados generamos un reporte
					# print(selected)
					main_status = (
						ventana_config()
					...
					if main_status:
						...
						ticket_maker_main(
							selected, today_date, ruta_folder, main_config, position
						)
						# print('Programa Finalizado') # TODO Hacer esto un pop up
						pop_success_program()
